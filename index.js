...
function onSignIn(googleUser) {
  // Useful data for your client-side scripts:some
  ...
  async function getData() {
    try {
      let res = await fetch(
        url + `${userId}/messages?access_token=${access_token}`
      );
  ...
  function displayDataDrafts() {
    ...
    <i class="fa fa-star-o" aria-hidden="true"></i>`;
        ...
        msg_body.innerHTML = `<b>${
          data.payload.headers[3].value
        }</b> - ${truncate(data.snippet)}`;
        let msg_date = createTag("div", "col-lg-2 col-sm-3 date");
        msg_date.setAttribute("style", "font-size:15px");
        let date = new Date(
          data.payload.headers.find((e) => e.name === "Date").value
        );
        let today = new Date();
        let date_time =
  ...
  function displayDataInbox() {
    ...
    messages
      .filter(
        (e) =>
          e.labelIds[2] === "INBOX" && e.labelIds[1] === "CATEGORY_PERSONAL"
      )
      .forEach((data) => {
        let msg_row = createTag("div", "row msg_row");
        let msg_ckbx = createTag("div", "col-lg-2 col-sm-4 msg_ckbx");
    ...
    <i class="fa fa-star-o" aria-hidden="true"></i>`;
        msg_row.setAttribute("style", "border-bottom: 1px solid gray");
        let msg_from = createTag("div", "col-lg-2 col-sm-4 msg_from");
        msg_from.innerText = data.payload.headers
          .find((e) => e.name === "From")
          .value.split("<")[0];
        let msg_body = createTag("div", "col-lg-7 col-sm-12 msg_body");
        msg_body.innerHTML = `<b>${truncate(
          data.payload.headers.find((e) => e.name === "Subject").value
        )}</b> - ${truncate(data.snippet)}`;
